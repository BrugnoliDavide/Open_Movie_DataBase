                        ...
                        onDelete = { reviewToDelete ->
                            coroutineScope.launch(Dispatchers.IO) {
                                ...
                                withContext(Dispatchers.Main) {
                                    ...
                                    else updated.filter { it.title.contains(searchQuery.text, ignoreCase = true) }
                                }
                            }
                        },
                        onRate = { reviewToRate: Review, newRating: Float ->
                            coroutineScope.launch(Dispatchers.IO) {
                                ...
                                withContext(Dispatchers.Main) {
                                    ...
                                    filteredFavorites = if (searchQuery.text.isEmpty()) updatedList
                                    else updatedList.filter { it.title.contains(searchQuery.text, ignoreCase = true) }
                                }
                            }
                                 },
...
fun FavoriteItem(
    ...
    Box(
        ...
        Card(
            ...
            Column(modifier = Modifier.padding(16.dp)) {
                Text(text = review.title, style = MaterialTheme.typography.titleMedium, color = MaterialTheme.colorScheme.onSurface)
    ...
    if (showRatingDialog) {
        AlertDialog(
            ...
            title = { Text(text = "Inserisci valutazione", color = MaterialTheme.colorScheme.onSurface) },
